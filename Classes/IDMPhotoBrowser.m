...
@property (nonatomic, strong) UIActionSheet *deletionSheet;
...
@synthesize actionsSheet = _actionsSheet, activityViewController = _activityViewController, deletionSheet = _deletionSheet;
...
- (void)deleteButtonPressed:(id)sender {
    id <IDMPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        _deletionSheet = [[UIActionSheet alloc]
                          initWithTitle:nil
                          delegate:self
                          cancelButtonTitle:@"Cancel"
                          destructiveButtonTitle:@"Delete photo"
                          otherButtonTitles:nil];
        [_deletionSheet showInView:self.view];
    }
}
